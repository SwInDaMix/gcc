...
#define BUZZER_GET_TEMP_TICKS(x) (60.0 * PERIODIC_TICKS_PER_SECOND / 20 / x)
...
#define BUZZER_DELAY (6 * PERIODIC_TICKS_PER_SECOND / 1000)

static sBuzzer_Interface const *g_buzzer_interface;

static sBuzzerQuant const *volatile g_buzzer_melody_ptr;
static size_t volatile g_buzzer_melody_length;
static int16_t volatile g_buzzer_quant_duration;

void Buzzer_Init(sBuzzer_Interface const *buzzer_interface) { g_buzzer_interface = buzzer_interface; }

void Buzzer_Periodic() {
    if (!g_buzzer_interface) return;

    if(g_buzzer_melody_ptr) {
        if(g_buzzer_quant_duration > 0) {
            if(!(--g_buzzer_quant_duration)) {
                g_buzzer_interface->set_silence();
                g_buzzer_quant_duration = -BUZZER_DELAY;
            }
        }
        else if(g_buzzer_quant_duration < 0) {
            if(!(++g_buzzer_quant_duration)) {
                if(g_buzzer_melody_length) {
                    g_buzzer_melody_length--;
                    sBuzzerQuant _quant = *g_buzzer_melody_ptr++;
                    eBuzzerNote _quant_note = (eBuzzerNote)_quant.Note;
                    g_buzzer_quant_duration = (eBuzzerNote)(_quant.Duration << 2) - BUZZER_DELAY;
                    g_buzzer_interface->set_wave(_quant_note, _quant.Octave);
                }
            }
        }
        else { g_buzzer_interface->set_silence(); g_buzzer_melody_ptr = nullptr; }
    }
    else { g_buzzer_interface->set_silence(); }
}

void Buzzer_PlayMelody(sBuzzerQuant const *melody, size_t length) {
    __extension__ void _block() { g_buzzer_quant_duration = -1; g_buzzer_melody_ptr = melody; g_buzzer_melody_length = length; }

    g_buzzer_interface->irq_safe(_block);
}
void Buzzer_Stop() { g_buzzer_melody_ptr = nullptr; }

void Buzzer_Beep(sBuzzerQuant quant) {
    static sBuzzerQuant __beep_melody;
    __beep_melody = quant;
    Buzzer_PlayMelody(&__beep_melody, 1);
}
