...
static uint8_t s_motor_characteristic;
...
static void _ktlcd3_reparse_struct() {
    if (s_need_update) {
        ...
        s_motor_characteristic = s_received_struct.p1;
        ...
        s_handlebar_function = (s_received_struct.c4 & KTLCD3_Masks_c4) >> KTLCD3_Shifts_c4;
...
uint8_t ktlcd3_get_motor_characteristic() { _ktlcd3_reparse_struct(); return s_motor_characteristic; }
eKTLCD3_WheelSize ktlcd3_get_wheel_size() { _ktlcd3_reparse_struct(); return s_wheel_size; }
float ktlcd3_get_wheel_size_circumference() { _ktlcd3_reparse_struct(); return s_wheel_size_lookup[s_wheel_size]; }
...
uint8_t ktlcd3_get_max_current_scale() { _ktlcd3_reparse_struct(); return s_max_current_lookup[s_max_current]; }
eKTLCD3_MinVoltage ktlcd3_get_min_voltage() { _ktlcd3_reparse_struct(); return s_min_voltage; }
int8_t ktlcd3_get_min_voltage_correct() { _ktlcd3_reparse_struct(); return s_min_voltage_correct_lookup[s_min_voltage]; }
...
bool ktlcd3_receivebyte(uint8_t byte) {
    ...
    static uint8_t s_received_buf_crc = 0;
    static bool s_fullbuf = false;

    uint8_t _crc_idx;
    uint8_t _s_received_buf_idx_next;

    _crc_idx = s_received_buf_idx + (uint8_t)7;
    if (_crc_idx >= sizeof(sKTLCD3_From)) _crc_idx -= sizeof(sKTLCD3_From);
    s_received_buf_crc ^= s_received_buf[s_received_buf_idx];
    s_received_buf_crc ^= s_received_buf[_crc_idx];
    ...
    if (s_fullbuf && s_received_buf_crc == s_received_buf[_crc_idx]) {
        // crc match
        _s_received_buf_idx_next = s_received_buf_idx;
        if (_s_received_buf_idx_next) _s_received_buf_idx_next--; else _s_received_buf_idx_next = sizeof(sKTLCD3_From) - 1;
        if (s_received_buf[_s_received_buf_idx_next] == 14) {
            if (_s_received_buf_idx_next) _s_received_buf_idx_next--; else _s_received_buf_idx_next = sizeof(sKTLCD3_From) - 1;
            if (s_received_buf[_s_received_buf_idx_next] == 50) {
                // header markers match
...
void ktlcd3_update(eKTLCD3_Battery battery, eKTLCD3_Error error, eKTLCD3_MovingStatus moving_status, uint16_t wheel_period_ms, uint8_t motor_current, int8_t motor_temperature) {
    ...
    _to_send.wheel_period_ms = swap16(wheel_period_ms);
