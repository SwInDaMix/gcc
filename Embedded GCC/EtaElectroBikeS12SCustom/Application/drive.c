...
#define DRV_MAKE_SMOOTH(x) (((uint8_t)x << 8) | (uint8_t)x)
...
#define DRV_ACCELERATION_SMOOTH_UNIT DRV_MAKE_SMOOTH(1)
...
void drv_cycle(sSettings const *settings, sControls const *controls, sSensors const *sensors) {
    ...
    static uint8_t _s_prev_duty_cycle, _s_predicted_duty_cycle;
    static uint8_t _s_new_duty_cycle;
    static uint16_t _s_new_duty_cycle_smooth;
    ...
    if ((_timer_current - _s_timer_prev) > TIMER_TP_MS(DRV_CYCLE_PERIOD_MS)) {
        ...
        bool _is_wanna_ebrake, _is_wanna_accelerate;

        _s_timer_prev = _timer_current;

        _current_erps = bldc_get_erps();
        _limit_current = ((uint16_t)DRV_MAX_CURRENT_025A * settings->max_current_scale) / (uint8_t)UINT8_MAX;
        // scale max current according to gear ratio if needed
        if(settings->scale_max_current_by_gear) _limit_current = map16(controls->gear_ratio, 0, UINT8_MAX, 0, _limit_current);
        ...
        if (s_is_auto_stabilization) {
            ...
            if (s_state != DRV_STATE_IDLE && (_current_erps >= DRV_MIN_ERPS_TO_PREDICT && _s_prev_duty_cycle >= DRV_MIN_DUTY_CYCLE_TO_PREDICT)) {
        ...
        if (controls->is_braking) {
            // If we are braking, there is only one option - reset all and turn off BLDC!
            s_state = DRV_STATE_IDLE;
            _s_new_duty_cycle_smooth = 0;
            s_cruise_control_min_duty_cycle = 0;
        } else {

            // what we want
            _is_wanna_ebrake = controls->thumb_brake >= DRV_THUMB_TOGGLE_THRESHOLD;
            _is_wanna_accelerate = controls->thumb_throttle >= DRV_THUMB_TOGGLE_THRESHOLD;

            if(_is_wanna_ebrake || _is_wanna_accelerate) {
                uint16_t _duty_cycle_diff;
                uint16_t _limit_erps;
                uint16_t _max_duty_cycle_smooth = DRV_MAKE_SMOOTH(controls->gear_ratio);

                // reset new duty cycle if starting to drive
                if(s_state == DRV_STATE_IDLE) _s_new_duty_cycle_smooth = DRV_MAKE_SMOOTH(_s_predicted_duty_cycle < DRV_START_DUTY_CYCLE ? (uint8_t)DRV_START_DUTY_CYCLE : _s_predicted_duty_cycle);
                // if we are trying to accelerate or ebrake after cruise control idle, then turn off cruise
                if(s_is_cruise_control_idle) { s_is_cruise_control_idle = false; s_cruise_control_min_duty_cycle = 0; }
                ...
                if (sensors->motor_current > _limit_current) {
                    ...
                    if(s_state == DRV_STATE_EBRAKING) { if (_s_new_duty_cycle_smooth < (_max_duty_cycle_smooth - DRV_MAX_ACCELERATION_RAMP_SMOOTH)) _s_new_duty_cycle_smooth += DRV_MAX_ACCELERATION_RAMP_SMOOTH; else _s_new_duty_cycle_smooth = _max_duty_cycle_smooth; }
                    else if(s_state == DRV_STATE_ACCELERATING) { if (_s_new_duty_cycle_smooth > DRV_MAX_ACCELERATION_RAMP_SMOOTH) _s_new_duty_cycle_smooth -= DRV_MAX_ACCELERATION_RAMP_SMOOTH; else _s_new_duty_cycle_smooth = 0; }
                }
                else if(_limit_erps != 0 && _current_erps > _limit_erps) {
                    // if speed is above the limited, then slowly decrease duty cycle
                    if(_s_new_duty_cycle_smooth >> 8) _s_new_duty_cycle_smooth -= DRV_ACCELERATION_SMOOTH_UNIT; else _s_new_duty_cycle_smooth = 0;
                }
                else if(s_cruise_control_min_duty_cycle) {
                    // if on cruise control
                    static uint8_t _s_prev_gear_ratio;

                        // if gear ratio changed, turn of cruise control
                        if (_s_prev_gear_ratio != controls->gear_ratio) s_cruise_control_min_duty_cycle = 0;
                        else {
                            // otherwise keep duty cycle as requested by cruise control
                            _s_new_duty_cycle_smooth = s_cruise_control_min_duty_cycle << 8;
                            // but not less than predicted duty cycle minus threshold
                            // TODO: do we need this?
                            // if (_s_predicted_duty_cycle > _s_new_duty_cycle_smooth && (_s_predicted_duty_cycle - _s_new_duty_cycle_smooth) >= DRV_THUMB_TOGGLE_THRESHOLD) _s_new_duty_cycle_smooth = _s_predicted_duty_cycle - (uint8_t)(DRV_THUMB_TOGGLE_THRESHOLD - 1);
                        }

                    _s_prev_gear_ratio = controls->gear_ratio;
                }
                else if(_is_wanna_ebrake) {
                    // wa are free to decrease duty cycle for ebraking
                    if(s_state != DRV_STATE_EBRAKING) { if(_s_new_duty_cycle_smooth > DRV_EBRAKE_BOOST_SMOOTH) _s_new_duty_cycle_smooth -= DRV_EBRAKE_BOOST_SMOOTH; else _s_new_duty_cycle_smooth = 0; }
                    s_state = DRV_STATE_EBRAKING;
                    _duty_cycle_diff = (uint16_t)((uint32_t)DRV_MAX_ACCELERATION_RAMP_SMOOTH * controls->thumb_brake / UINT8_MAX);
                    if(_s_new_duty_cycle_smooth > _duty_cycle_diff) _s_new_duty_cycle_smooth -= _duty_cycle_diff; else _s_new_duty_cycle_smooth = 0;
                }
                else if(_is_wanna_accelerate) {
                    // wa are free to increase duty cycle for ebraking
                    if(s_state != DRV_STATE_ACCELERATING) { if(_s_new_duty_cycle_smooth < (_max_duty_cycle_smooth - DRV_ACCELERATION_BOOST_SMOOTH)) _s_new_duty_cycle_smooth += DRV_ACCELERATION_BOOST_SMOOTH; else _s_new_duty_cycle_smooth = _max_duty_cycle_smooth; }
                    s_state = DRV_STATE_ACCELERATING;
                    _duty_cycle_diff = controls->is_walking ? (uint16_t)DRV_MAX_ACCELERATION_RAMP_SMOOTH : (uint16_t)((uint32_t)DRV_MAX_ACCELERATION_RAMP_SMOOTH * controls->thumb_throttle / UINT8_MAX);
                    if(_s_new_duty_cycle_smooth < (_max_duty_cycle_smooth - _duty_cycle_diff)) _s_new_duty_cycle_smooth += _duty_cycle_diff; else _s_new_duty_cycle_smooth = _max_duty_cycle_smooth;
                }
            }
            else {
                // if idling during cruise_control, note it
                if(s_cruise_control_min_duty_cycle) s_is_cruise_control_idle = true;
                else {
            ...
            _s_new_duty_cycle = (uint8_t)(_s_new_duty_cycle_smooth >> 8);

            if (s_state != DRV_STATE_IDLE && s_cruise_control_min_duty_cycle == 0 && controls->is_cruise_control) {
                // setup cruise control if requested
