 ...
 */
static uint8_t const stm_reset_code[] = {
...
static uint32_t const stm_reset_code_length = sizeof(stm_reset_code);
...
/* internal functions */
uint8_t stm32_gen_cs(uint32_t const v);
void    stm32_send_byte(stm32_t const *stm, uint8_t byte);
uint8_t stm32_read_byte(stm32_t const *stm);
char    stm32_send_command(stm32_t const *stm, uint8_t const cmd);
...
stm32_t *stm32_init(serial_t const *serial, char const init) {
	uint8_t len, val;
	stm32_t *stm;
...
char stm32_read_memory(stm32_t const *stm, uint32_t address, uint8_t data[], unsigned int len) {
	uint8_t cs;
	unsigned int i;
...
char stm32_write_memory(stm32_t const *stm, uint32_t address, uint8_t const data[], unsigned int len) {
	uint8_t cs;
	unsigned int i;
	int c, extra;
	assert(len > 0 && len < 257);
...
char stm32_wunprot_memory(stm32_t const *stm) {
...
char stm32_runprot_memory  (stm32_t const *stm) {
...
char stm32_readprot_memory(stm32_t const *stm) {
...
char stm32_erase_memory(stm32_t const *stm, uint16_t spage, uint16_t pages) {
	...
	if (stm->cmd->er == STM32_CMD_EE) {
		...
		if (stm->pid == 0x416 && pages == 0xFFFF) pages = 0xF8; /* works for the STM32L152RB with 128Kb flash */
...
char stm32_run_raw_code(stm32_t const *stm, uint32_t target_address, uint8_t const *code, uint32_t code_size)
{
...
char stm32_go(stm32_t const *stm, uint32_t address) {
	uint8_t cs;
...
char stm32_reset_device(stm32_t const *stm) {
