...
int main(int argc, char *argv[]) {
	...
	if (rd) {
		...
		fprintf(stderr, "Specified start 0x%08X, end 0x%08X start page %d, n pages %d\n", start, end, spage, npages);
		...
		while(addr < end && offset < size) {
			...
			if (parser->read(p_st, buffer, (unsigned int *)&len) != PARSER_ERR_OK)
				goto close;
