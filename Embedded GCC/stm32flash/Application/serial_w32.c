...
serial_t *serial_open(char const *device)
{
	...
	COMMTIMEOUTS timeouts = {MAXDWORD, MAXDWORD, 6000, 0, 0};
...
serial_err_t serial_setup(serial_t *h,
			  serial_baud_t const baud,
			  serial_bits_t const bits,
			  serial_parity_t const parity,
			  serial_stopbit_t const stopbit)
{
	assert(h && h->fd != INVALID_HANDLE_VALUE);
